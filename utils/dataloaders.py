...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, (
            f"No images or videos found in the specified path. "
            f"Supported formats are:\nimages: {IMG_FORMATS}\nvideos: {VID_FORMATS}"
        )
    ...
    def __next__(self):
        ...
        # Read two images (lwir and visible)
        lwir_path = self.files[self.count].replace('{}', 'lwir')
        visible_path = self.files[self.count].replace('{}', 'visible')
        
        im0_lwir = cv2.imread(lwir_path)  # BGR
        im0_visible = cv2.imread(visible_path)  # BGR
        
        assert im0_lwir is not None, f"Image Not Found {lwir_path}"
        assert im0_visible is not None, f"Image Not Found {visible_path}"
        
        self.count += 1
        s = f"image {self.count}/{self.nf} {lwir_path}, {visible_path}: "

        if self.transforms:
            im_lwir = self.transforms(im0_lwir)  # transforms
            im_visible = self.transforms(im0_visible)  # transforms
        else:
            im_lwir = letterbox(im0_lwir, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize
            im_lwir = im_lwir.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im_lwir = np.ascontiguousarray(im_lwir)  # contiguous

            im_visible = letterbox(im0_visible, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize
            im_visible = im_visible.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im_visible = np.ascontiguousarray(im_visible)  # contiguous

        # Merge two images along the channel dimension
        im = np.concatenate((im_lwir, im_visible), axis=0)

        return (lwir_path, visible_path), im, (im0_lwir, im0_visible), self.cap, s
